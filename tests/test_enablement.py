...
def _already_enabled_cdb_oci(db_status: str) -> FakeOci:
    return FakeOci(
        ...
        insights=[
            {"database-id": "database-id", "lifecycle-state": "ACTIVE"}
        ],  # skip OPSI create noise
        db_status=db_status,
    )
...
def test_enable_force_reconcile_modifies_even_when_healthy() -> None:
    oci = _already_enabled_cdb_oci(db_status="UP")
    EnablementService(oci).enable_target(
        _cdb_already_enabled_target(), force_reconcile=True
...
def test_enable_cloud_database_reraises_untolerated_error() -> None:
    oci = FakeOci(
        fail_on_index=0, fail_text="ServiceError: LimitExceeded quota reached"
    )
    service = EnablementService(oci)  # type: ignore[arg-type]
    target = Target(
        kind="dbcs",
...
def test_enable_dbm_fails_when_control_plane_reaches_failed_enabling() -> None:
    class FailedDbmOci(FakeOci):
        def get_database(self, database_id):
            return {
                "database-management-config": {
                    "management-status": "FAILED_ENABLING",
                }
            }

    service = EnablementService(FailedDbmOci())  # type: ignore[arg-type]
    target = Target(
        kind="dbcs",
        name="db1",
        resource_id="database-id",
    ...
    try:
        service.enable_dbm(target)
    except RuntimeError as exc:
        assert "FAILED_ENABLING" in str(exc)
        assert "db1" in str(exc)
    else:
        raise AssertionError("Expected terminal DBM failure to fail the enablement run")


def test_enable_dbm_reconcile_verifies_terminal_control_plane_status() -> None:
    class FailedReconcileOci(FakeOci):
        def __init__(self):
            super().__init__(
                fail_on_index=0,
                fail_text=(
                    "IncorrectState: Either DatabaseManagement is already enabled "
                    "or request to enable it is already created."
                ),
                db_status="DOWN",
            )
    ...
    service = EnablementService(FailedReconcileOci())  # type: ignore[arg-type]

    try:
        service.enable_dbm(_cdb_already_enabled_target())
    except RuntimeError as exc:
        assert "FAILED_ENABLING" in str(exc)
    else:
        raise AssertionError("Expected failed DBM reconcile to fail the enablement run")


def test_enable_skips_opsi_create_when_insight_already_active(caplog) -> None:
    oci = FakeOci(
        insights=[{"database-id": "database-id", "lifecycle-state": "ACTIVE"}]
    )
    service = EnablementService(oci)  # type: ignore[arg-type]
    caplog.set_level(logging.INFO, logger="dbman_opsi.enablement")
    target = Target(
...
def test_enable_opsi_repairs_existing_failed_insight_instead_of_recreating() -> None:
    oci = FakeOci(
        insights=[
            {
                "id": "failed-insight-id",
    ...
    assert "failed-insight-id" in oci.commands[0]
    assert "db1" in oci.commands[0]
    assert "VAULT_SECRET" not in oci.commands[0]
    assert not any("create-pe-comanged-database" in command for command in oci.commands)
...
class _OpsiCreateOci:
    ...
    def run_tolerating(self, args, tolerated):
        ...
        if self.create_calls <= self.fail_times:
            raise RuntimeError(
                "400-MissingParameter, Provided database resource details were missing."
            )
        return True
...
def test_opsi_create_retries_on_propagation_then_succeeds() -> None:
    ...
    service = EnablementService(
        oci,
        opsi_create_attempts=5,
        opsi_create_delay=0.0,
    ...
    target = Target(
        ...
        resource_id="db-1",
        service_name="svc",
        private_endpoint_id="dbmpe",
        opsi_private_endpoint_id="opsipe",
        opsi_credential_details_file="cred.json",
...
def test_opsi_create_raises_after_exhausting_propagation_retries() -> None:
    ...
    service = EnablementService(
        oci, opsi_create_attempts=3, opsi_create_delay=0.0, sleeper=lambda d: None
    )  # type: ignore[arg-type]
    target = Target(
        kind="dbcs",
        name="cdb",
...
def test_wait_dbm_enabled_fails_when_enabling_never_completes() -> None:
    oci = _DbmWaitOci(["ENABLING"])
    service = EnablementService(oci, sleeper=lambda _delay: None)  # type: ignore[arg-type]
    service.dbm_wait_attempts = 2
    service.dbm_wait_delay = 0.0

    try:
        service._wait_dbm_enabled(
            Target(kind="dbcs", name="cdb", resource_id="db-1", database_role="CDB")
        )
    except TimeoutError as exc:
        assert "cdb" in str(exc)
    else:
        raise AssertionError("Expected a non-progressing DBM enablement to time out")
...
def test_wait_dbm_enabled_is_best_effort_when_unreadable() -> None:
    ...
    service._wait_dbm_enabled(
        Target(kind="dbcs", name="cdb", resource_id="db-1")
    )  # must not raise
