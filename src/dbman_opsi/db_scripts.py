...
def host_firewall_script(target: Target) -> str:
    ports = " ".join(str(port) for port in HOST_FIREWALL_PORTS)
...
# Source CIDR should be the private endpoint subnet, DB subnet, or approved
# operator/Bastion CIDR. Leave empty to print broad port commands for lab-only
# environments; production should set this before --apply.
SOURCE_CIDR="${{DBMAN_OPSI_SOURCE_CIDR:-}}"
PORTS="${{DBMAN_OPSI_DB_PORTS:-{ports}}}"
...
echo "Checking host firewall for Oracle listener ports: $PORTS"
echo "Set DBMAN_OPSI_SOURCE_CIDR to restrict firewall rules to the monitoring source CIDR."

if command -v firewall-cmd >/dev/null 2>&1 && sudo firewall-cmd --state >/dev/null 2>&1; then
  echo "firewalld detected."
  for port in $PORTS; do
    if sudo firewall-cmd --query-port="${{port}}/tcp" >/dev/null 2>&1; then
      echo "PASS: firewalld allows $port/tcp in the active zone."
    else
      echo "MISSING: firewalld does not allow $port/tcp in the active zone."
    fi
    if [ -n "$SOURCE_CIDR" ]; then
      run_or_print sudo firewall-cmd --permanent --add-rich-rule="rule family=\\"ipv4\\" source address=\\"${{SOURCE_CIDR}}\\" port port=\\"${{port}}\\" protocol=\\"tcp\\" accept"
    else
      run_or_print sudo firewall-cmd --permanent --add-port="${{port}}/tcp"
    fi
  done
  run_or_print sudo firewall-cmd --reload
  exit 0
fi
...
if command -v iptables >/dev/null 2>&1; then
  ...
  for port in $PORTS; do
    if sudo iptables -C INPUT -p tcp --dport "$port" -j ACCEPT >/dev/null 2>&1; then
      echo "PASS: iptables allows $port/tcp."
    else
      echo "MISSING: iptables does not allow $port/tcp."
    fi
    if [ -n "$SOURCE_CIDR" ]; then
      run_or_print sudo iptables -I INPUT -p tcp -s "$SOURCE_CIDR" --dport "$port" -j ACCEPT
    else
      run_or_print sudo iptables -I INPUT -p tcp --dport "$port" -j ACCEPT
    fi
  done
  if command -v service >/dev/null 2>&1; then
    run_or_print sudo service iptables save
  fi
  exit 0
fi

echo "No supported host firewall command found. Manually allow TCP ports: $PORTS"
exit 2
"""
...
0. `00-check-host-firewall.sh` (host OS firewall; read-only unless `--apply`)
...
First verify the host OS firewall on each DB node. The script prints remediation
commands by default and applies them only when called with `--apply`:
...
Run the database scripts from SQL*Plus or SQLcl as SYSDBA or an equivalent administrative account:
...
def generate_db_scripts(config: EnablementConfig, output_dir: str | Path) -> list[Path]:
    ...
    for target in config.targets:
        ...
        files = {
            ...
            "00-check-host-firewall.sh": host_firewall_script(target),
