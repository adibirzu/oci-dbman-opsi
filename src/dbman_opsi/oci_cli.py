...
class OciCli:
    ...
    # OPSI database-insights list excludes FAILED/terminal states by default,
    # so pass the full set explicitly to surface broken insights during validate.
    ...
    def list_opsi_database_insights(self, compartment_id: str) -> list[dict[str, Any]]:
        ...
        for state in self.OPSI_INSIGHT_STATES:
            args.extend(["--lifecycle-state", state])
        return self._items(self.run_json(args))
